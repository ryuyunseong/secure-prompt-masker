# secure-prompt-masker

AI 도구, 이슈, 보고서, 메신저에 로그나 요청 데이터를 붙여넣기 전에 민감한 텍스트를 로컬에서 마스킹하는 Python/Tkinter 데스크톱 유틸리티입니다.

## 실행 화면

![가짜 HTTP 요청의 민감정보를 로컬에서 탐지하고 마스킹한 실행 화면](docs/images/secure-prompt-masker-demo.png)

> 화면의 요청 데이터와 인증정보는 모두 시연용 가짜 값입니다.

## 주요 기능

- 별도 서버 없이 로컬 데스크톱 앱으로 실행
- 토큰, 쿠키, 인증 헤더, 비밀번호성 키-값, 전화번호 형태 값 등 공통 민감 패턴 탐지
- 프로젝트별 사용자 지정 키워드 마스킹
- 요청 조각을 다룰 때 사용할 수 있는 URL 인코딩 모드
- 로컬 설정 파일은 Git에 포함되지 않도록 관리
- 저장소 예시는 모두 가짜 값만 사용

> URL 인코딩은 문자의 표현만 바꾸는 가역 변환이며 민감정보를 숨기지 않습니다. 외부 공유 전에는 마스킹 모드를 사용하세요.

## 만든 이유

보안 점검, 디버깅, 장애 분석, AI 보조 업무에서는 요청 헤더, 쿠키, 토큰, 계정 식별자, 내부 프로젝트명, 고객 맥락이 섞인 텍스트를 다루는 일이 많습니다. `secure-prompt-masker`는 이런 텍스트를 외부 도구에 옮기기 전에 로컬에서 한 번 검토하고 마스킹하도록 돕습니다.

## 마스킹 처리 흐름

```mermaid
flowchart LR
    A[원본 텍스트 입력] --> B[공통 민감 패턴 탐지]
    B --> C[사용자 확인 및 선택]
    C --> D[HTTP·JSON·XML 구조 필드 마스킹]
    D --> E[사용자 지정 키워드 마스킹]
    E --> F[잔여 민감 패턴 재검사]
    F -->|잔여값 발견| G[자동 추가 마스킹]
    F -->|잔여값 없음| H[결과 검토 및 복사]
    G --> H
```

## 설계 포인트

- 네트워크 요청 코드나 외부 서비스 의존성 없이 Python 표준 라이브러리와 Tkinter만 사용합니다.
- 일반 민감 패턴 탐지, HTTP/JSON/XML 구조화 필드 마스킹, 사용자 지정 키워드 치환을 순서대로 적용합니다.
- 변환 후 민감 패턴을 다시 검사하고, 남은 값은 한 번 더 자동 마스킹합니다.
- 탐지한 원문 값은 파일에 저장하지 않으며, 로컬 설정에는 UI 환경과 사용자 지정 키워드만 보관합니다.

## 프로젝트 구조

~~~text
secure_prompt_masker.py          GUI, 탐지, 마스킹, 로컬 설정
tests/test_masking.py            핵심 마스킹 로직 회귀 테스트
docs/                            사용법, 샘플, 보안 및 공개 점검 문서
SECURITY.md                      보안 이슈 보고 원칙
LICENSE                          MIT License
~~~

## 실행 방법

필요 조건:

- Python 3.10 이상
- Python 설치에 Tkinter 지원 포함

저장소 루트에서 실행합니다.

```bash
python secure_prompt_masker.py
```

기본 사용 흐름:

1. 입력 영역에 텍스트를 붙여넣습니다.
2. 탐지된 민감값을 확인합니다.
3. 마스킹할 값 또는 분류를 선택합니다.
4. 결과를 직접 검토한 뒤 복사합니다.

## 안전한 데모 입력

```text
POST /api/demo/action
Authorization: Bearer 데모토큰값
Cookie: session=데모세션값
X-Api-Key: 데모API키값
project=예시프로젝트
owner=샘플담당자
message=AI 보조 업무에 사용하기 전에 이 텍스트를 마스킹합니다.
```

출력 예시:

```text
POST /api/demo/action
Authorization: Bearer ******
Cookie: session=******
X-Api-Key: ******
project=******
owner=*****
message=AI 보조 업무에 사용하기 전에 이 텍스트를 마스킹합니다.
```

위 값은 모두 가짜입니다. 실제 회사명, 고객 데이터, 인증정보, 도메인, 전화번호, 주소, 운영 로그, 비공개 프롬프트를 이 저장소에 추가하지 마세요.

## 검증

기본 구문 검사를 실행합니다.

```bash
python -m py_compile secure_prompt_masker.py
```

자동 회귀 테스트를 실행합니다.

~~~bash
python -m unittest discover -s tests -v
~~~

권장 수동 스모크 테스트:

1. 앱을 실행합니다.
2. 위 데모 입력을 붙여넣습니다.
3. 마스킹을 실행합니다.
4. 가짜 토큰, 쿠키, API 키, 사용자 지정 키워드가 마스킹되는지 확인합니다.
5. 앱 종료 후 로컬 설정/출력 파일이 커밋 대상에 들어가지 않았는지 확인합니다.

## 개인정보와 보안 메모

- 처리는 로컬에서 수행하는 것을 전제로 합니다.
- 마스킹 결과에도 민감한 맥락이 남을 수 있으므로 공유 전 직접 검토해야 합니다.
- 클립보드 내용은 다른 로컬 소프트웨어에 노출될 수 있습니다.
- 로컬 설정, 캡처, 내보내기, 로그, 스크린샷은 커밋하지 않아야 합니다.
- 패턴 기반 마스킹은 보조 안전장치이며 완전한 보장을 의미하지 않습니다.

## 한계

- 드문 형식의 비밀값은 탐지하지 못할 수 있습니다.
- 정상 식별자가 민감값으로 오탐될 수 있습니다.
- 이 도구는 안전한 데이터 처리 절차, DLP 검토, 법무/컴플라이언스 승인을 대체하지 않습니다.
- 공개 전에는 현재 파일뿐 아니라 Git 히스토리까지 점검해야 합니다.

## 문서

- [사용 가이드](docs/USAGE.md)
- [샘플 데이터](docs/SAMPLES.md)
- [개인정보와 보안](docs/PRIVACY_AND_SECURITY.md)
- [공개 전 체크리스트](docs/PUBLIC_RELEASE_CHECKLIST.md)
- [보안 정책](SECURITY.md)

## 라이선스

이 프로젝트는 MIT License로 공개합니다. 자세한 내용은 [LICENSE](LICENSE)를 확인하세요.
